...
function dados() {
  ...
  return (
    `nome=${frm.nome.value}&nascimento=${frm.data_nasc.value}&cpf=${frm.cpf.value}&` +
...
function valida_input() {
  ...
  inputs.forEach((element) => {
    if (
      element.value === "" &&
      element.attributes.name.value != "complemento"
    ) {
      retorno = false;
    }
  });
...
function sendCadastro(e) {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.status == 200 && xhr.readyState == 4) {
      ...
      ajaxResponse.textContent = responseJson.dados;
      setTimeout(() => {
        window.location.href = "http://poupemais.com/login";
      }, 2000);
    }
  };
